...
const SearchBox = React.forwardRef<HTMLInputElement, SearchProps>(
  ({ className, onSubmit, onClear, ...rest }, ref) => {
    return (
      <form
        className="flex justify-between relative pe-12 md:pe-14 bg-white overflow-hidden rounded-md w-full"
        noValidate
        role="search"
        onSubmit={onSubmit}
      >
        ...
        <button
          type="button"
          className="outline-none text-2xl md:text-3xl text-gray-400 absolute right-0 top-0 end-0 w-12 md:w-14 h-full flex items-center justify-center transition duration-200 ease-in-out hover:text-heading focus:outline-none"
          onClick={onClear}
        >
